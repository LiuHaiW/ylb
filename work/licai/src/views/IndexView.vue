<template>
  ...
  <div class="banner-content">
    ...
    <div class="banner-abs">
      <div class="banner-abs-box">
        ...
        <b>{{ appinfo.averageRate }}<i>%</i></b>
        <p>平台用户数</p>
        <span>{{ appinfo.registerUsers }}<i>位</i></span>
        <p class="banner-abs-border">累计成交金额</p>
        <span>{{ appinfo.allBidMoney }}<i>元</i></span>
      </div>
    </div>
  </div>
  ...
  <div class="content">
    ...
    <div class="new-user" v-for="product in xinVoList" :key="product.id">
    ...
    <ul class="preferred-select clearfix">
      <li v-for="(product,index) in youVoList" :key="product.id">
    ...
    <h2 class="public-title"><span>散标产品</span>
      <a href="javascript:void(0)" @click="goLink('/product/list',{ptype:2})"
         class="public-title-more">查看更多产品>></a></h2>
    <ul class="preferred-select clearfix">
      <li v-for="product in sanVoList" :key="product.id">
...
export default {
  ...
  mounted() {//页面加载后执行
    ...
    //获取理财产品列表
    doGet('/product/index').then(resp => {
      if (resp.data.code === 1000) {
        this.xinVoList = resp.data.info.xinVoList;
        this.youVoList = resp.data.info.youVoList;
        this.sanVoList = resp.data.info.sanVoList;
      }
    })
